...
ann = myModel.Net(n_feature=2, n_hidden=800, n_output=1).to(device)
print(ann)
...
for epoch in range(500000):
    ...
    for batch in range(n_batches):
        ...
        optimizer_batch.zero_grad()    #  gradients
        loss.backward() # variation for each weight (and bias) of the network

        # we compute the new values for the weights
        optimizer_batch.step()

    if epoch % 10 == 0:
        predictedAnswer = ann(x)
        loss = lossFunction(predictedAnswer, f)
        print('\r epoch: {}\tLoss =  {:.5f}'.format(epoch, loss.item()))
