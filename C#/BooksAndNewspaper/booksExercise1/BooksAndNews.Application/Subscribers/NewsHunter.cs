...
namespace iQuest.BooksAndNews.Application.Subscribers
{
    ...
    public class NewsHunter
    {
        ...
        public NewsHunter(string name, PrintingOffice printingOffice, ILog log)
        {
            ...
            printingOffice.NewspaperPrinted += this.HandleNewsPrinted;
        }

        private void HandleNewsPrinted(object sender, NewspaperPrintedEventArgs args)
        {
            log.WriteInfo(String.Format("New newspaper printed for NewspaperHunter {0}, with title {1}, number {2} ", this.name, args.Newspaper.Title, args.Newspaper.Number));
        }
    }
}